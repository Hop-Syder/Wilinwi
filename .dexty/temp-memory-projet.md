...
- **[2026-09-13] [Vidage Mémoire BDD]** : Nettoyage intégral de toutes les données applicatives (28 tables vidées par TRUNCATE CASCADE) et suppression de 100% des comptes d'authentification Supabase Auth (`auth.users`). Rétablissement propre de la table de référence des plans (`plan_configs`). Base de données vierge et prête pour un cycle sain d'inscriptions.

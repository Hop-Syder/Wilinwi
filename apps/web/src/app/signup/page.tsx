    ...
    <main className="flex h-screen max-h-screen bg-background text-text-primary font-sans antialiased overflow-hidden">
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-between p-6 sm:p-10 h-screen max-h-screen bg-slate-50/50 overflow-hidden">
        ...
        <div className="w-full max-w-sm mx-auto my-auto space-y-4">
          ...
          <div className="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.01)] space-y-3.5">
            <form onSubmit={onSubmit} className="space-y-2.5">
              <div className="space-y-0.5">
                ...
                <Input 
                  ...
                  required 
                  className="w-full h-8.5 text-xs focus:ring-2 focus:ring-primary/20 transition-all"
                />
              </div>
              ...
              <Button type="submit" className="w-full justify-center h-9 text-xs font-semibold group rounded-xl shadow-md shadow-primary/5 hover:shadow-primary/10 transition-all mt-4" disabled={loading}>
                {loading ? 'Création en cours…' : (
                  <span className="flex items-center gap-1.5">
                    Créer ma boutique <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform" />
                  </span>
                )}
              </Button>
            </form>

...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6 pb-12">
      {/* ── AXE 1 : Top Bar & Contexte Temporel / Réseau (Alignement Single-Line Desktop) ── */}
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between pb-4 border-b border-slate-200/80">
        {/* BLOC GAUCHE : Titre + Établissement + Statut Réseau + Dates sur 1 seule ligne */}
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          {/* Badge Établissement */}
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-slate-100/80 text-slate-700 border border-slate-200/80 whitespace-nowrap shadow-2xs">
          ...
          {/* Badge Statut Réseau */}
          <span
            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold whitespace-nowrap border ${
              isOnline
          ...
          {/* Dates de la période */}
          <span className="text-xs text-slate-500 font-medium whitespace-nowrap hidden xl:inline-block pl-2 border-l border-slate-200">
            Période : <strong className="text-slate-800">{dateRange.from}</strong> au{' '}
            <strong className="text-slate-800">{dateRange.to}</strong>
          </span>
        </div>

        {/* BLOC DROIT : Filtres + Devise + Boutons d'actions sur 1 seule ligne */}
